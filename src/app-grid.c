...
static gboolean
is_filtered_app (PhoshAppGrid *self, const char *app_id, char * const *filtered_ids)
{
  PhoshAppGridPrivate *priv = phosh_app_grid_get_instance_private (self);
...
search_apps (gpointer item, gpointer data)
{
  ...
  if (G_IS_DESKTOP_APP_INFO (info)) {
    ...
    if (id && is_filtered_app (self, id, (char * const *)filtered_ids))
      return FALSE;
