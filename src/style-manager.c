...
/*
 * Relative weights of the glass surfaces, as multiples of glass-opacity.
 *
 * The ratios come from the alphas the stylesheets were hand-tuned at, which
 * are what the -furios sheets still carry as their fallbacks. That reference
 * set is quoted below against a sheet of .45, which is also glass-opacity's
 * default, so the shipped shell renders exactly the alphas quoted here. The
 * ratios only fix the surfaces against each other; glass-opacity sets the
 * weight of the whole set. Changing a ratio changes one surface relative to
 * the rest; changing the default changes all of them together.
 */
#define GLASS_RATIO_SHEET       1.0     /* top panel / drawer sheet, ref .45 */
#define GLASS_RATIO_TINT        0.556   /* settings menu layers,     ref .25 */
#define GLASS_RATIO_LOCK        1.111   /* lock screen sheet,        ref .50 */
#define GLASS_RATIO_FIELD       1.333   /* search bar,               ref .60 */
#define GLASS_RATIO_BUTTON      0.889   /* app grid buttons,         ref .40 */
...
struct _PhoshStyleManager {
  ...
  /* Pending application stylesheet, see schedule_app_stylesheet(). NULL css
   * with a live timeout means "take the file away". */
  guint           app_css_id;
  char           *app_css_pending;
};
...
/* How long a burst of glass setting changes is left to settle before the
 * application stylesheet is written */
#define APP_CSS_SETTLE_MS 250
...
/*
 * Write or remove the generated application stylesheet.
 *
 * Runs on a worker thread: this reads, writes and unlinks a file in the user's
 * config directory, and none of that belongs on the main loop of the shell
 * that draws the slider being dragged. Everything it needs is in @task_data,
 * so it touches no shell state and needs no locking.
 *
 * A file we did not write is left alone, in both directions.
 */
static void
app_stylesheet_write (GTask *task, gpointer source, gpointer task_data, GCancellable *cancellable)
{
  const char *css = task_data;
  ...
  if (css == NULL) {
    if (existing != NULL && g_unlink (path) != 0)
...
static gboolean
on_app_stylesheet_due (gpointer data)
{
  PhoshStyleManager *self = PHOSH_STYLE_MANAGER (data);
  g_autoptr (GTask) task = NULL;

  self->app_css_id = 0;

  task = g_task_new (NULL, NULL, NULL, NULL);
  g_task_set_task_data (task, g_steal_pointer (&self->app_css_pending), g_free);
  g_task_run_in_thread (task, app_stylesheet_write);
...
/*
 * Queue a write of the application stylesheet, or its removal when @css is
 * NULL.
 *
 * Coalesced rather than done there and then: every glass key emits its own
 * change, and dragging one slider in Settings emits a stream of them, so the
 * unthrottled version rewrote the same file dozens of times a second. Only the
 * last state of a burst is worth writing, and GTK4 reads the file once at
 * startup anyway, so nothing is lost by being a fraction of a second late.
 */
static void
schedule_app_stylesheet (PhoshStyleManager *self, char *css)
{
  g_free (self->app_css_pending);
  self->app_css_pending = css;

  if (self->app_css_id)
    g_source_remove (self->app_css_id);

  self->app_css_id = g_timeout_add (APP_CSS_SETTLE_MS, on_app_stylesheet_due, self);
  g_source_set_name_by_id (self->app_css_id, "[phosh] app_stylesheet");
}
 ...
 */
static void
remove_app_stylesheet (PhoshStyleManager *self)
{
  schedule_app_stylesheet (self, NULL);
}
                       ...
                       double             text_opacity)
{
  ...
  /* Same reference set as the shell's ratios above: quoted against a .45
   * sheet, scaled by whatever glass-opacity actually is. */
...
on_accent_color_changed (PhoshStyleManager *self)
{
   ...
   * text on an accent fill. Empty falls back to the theme's own foreground,
   * and glass-accent-text-color overrides just the accent case. */
   ...
   * glass: a selected quick setting, a pressed key, a card. glass-accent-text-color
   * sets it; empty leaves it following the shell's ordinary text colour. */
...
phosh_style_manager_dispose (GObject *object)
{
  ...
  g_clear_handle_id (&self->app_css_id, g_source_remove);
  g_clear_pointer (&self->app_css_pending, g_free);
